...
export const updateDynamoChatInfo = async (
  chatId: string,
  chatInfo: Chat,
): Promise<void> => {
  ...
  if (
    chatInfo &&
    chatStatistics &&
    !isEqual(chatStatistics.chatInfo, chatInfo)
  ) {
...
export const updateS3ChatInfo = async (
  chatId: string,
  chatInfo: Chat,
): Promise<void> => {
  const savedDataBuffer = await getFile(
    CHAT_DATA_BUCKET_NAME,
    String(chatId),
  ).catch(() => null)
  ...
  if (!isEqual(s3Data, chatInfo) && chatInfo) {
    await saveFile(
      CHAT_DATA_BUCKET_NAME,
      chatId,
      Buffer.from(JSON.stringify(chatInfo)),
    )
  }
}
